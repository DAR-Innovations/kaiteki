...
export class MembersListComponent implements OnInit, OnDestroy {
	...
	private loadTeamMembers() {
		...
		return this.teamsService.searchTeamMembers(pagination, this.filter).pipe(
			...
			map(res => res.content),
			catchError(err => {
				this.toastService.open('Failed to get team members')
	...
	onDeleteMember(id: number) {
		this.teamsService
			...
			.pipe(
				catchError(err => {
					this.toastService.open('Failed to delete team member')
					return throwError(() => err)
				}),
				take(1),
			)
			.subscribe(() => {
				this.toastService.open('Successfully removed team member')
				this.refetchTeamMembers()
			})
	}
}
