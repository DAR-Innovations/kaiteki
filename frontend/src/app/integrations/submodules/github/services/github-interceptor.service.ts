			...
			return next.handle(request).pipe(
				catchError(err => {
					if (err?.status === 401) {
						...
						this.toastService.error('GitHub integration is not connected')
						return throwError(() => err)
					}

					this.toastService.error('GitHub integration is not available')
					return throwError(() => err)
				}),
			)
		}
...
export const githubSessionInterceptorProvider = [
	{
